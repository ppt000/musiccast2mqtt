...
.. This document has been reviewed completely on 16May2018.

This is a guide on how to get started with the MusicCast to MQTT gateway.
It focuses on the particularities of the MusicCast interface.
There are other interfaces available in this project.
See the :doc:`Overview <overview>` for more details on the whole project.
...
1. Download the application package in a local directory.
2. Change a few parameters in the configuration file (mostly the details of the MQTT broker).
3. Define your audio-video system in a JSON file (the most important step).
4. Launch the process.

The first two and the last steps are explained in the :doc:`Installation <installation>` guide.
Simply change within the file names the term ``dummy`` into ``musiccast``.
The third step is specific to the MusicCast interface and is explained here.
...
functionality to do that.  However this would only work if all of the devices were *connected*
...
- the Device **id** is used to identify a device if it is defined in an incoming message;
  ...
  message via a mapping; see :doc:`concepts` for more details).
